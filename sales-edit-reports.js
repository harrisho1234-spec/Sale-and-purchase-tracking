...
(function(){
  ...
  window.showEditProductSuggestions=function(input){
    ...
    box.innerHTML=m.length?m.map(p=>`<button type="button" class="w-full text-left px-3 py-2 hover:bg-amber-50 border-b last:border-b-0" data-id="${esc(p.id)}" data-code="${esc(p.code||'')}" data-name="${esc(p.item_name||'')}" data-image="${esc(p.image_url||'')}" data-price="${Number(p.sales_price||0)}" onclick="chooseEditProduct(this)"><div class="text-xs font-bold text-[#a77d1a]">${esc(p.code||'No code')}</div><div class="text-sm truncate">${esc(p.item_name||'')}</div><div class="text-[10px] text-gray-400">${esc(p.brand||'')} ${p.class?'· '+esc(p.class):''} · ${money(p.sales_price||0,p.currency||'USD')}</div></button>`).join(''):'<div class="px-3 py-3 text-xs text-gray-400">No matching product.</div>';
    box.classList.remove('hidden');
  };
  window.editProductInputChanged=function(input){const r=input.closest('.edit-order-item-row');if(!r)return;r.querySelector('.edit-product-id').value='';r.querySelector('.edit-product-code').value='';r.querySelector('.edit-product-name').value='';r.querySelector('.edit-product-image').value='';showEditProductSuggestions(input)};
  window.chooseEditProduct=function(btn){
    const r=btn.closest('.edit-order-item-row');if(!r)return;
    r.querySelector('.edit-product-id').value=btn.dataset.id||'';r.querySelector('.edit-product-code').value=btn.dataset.code||'';r.querySelector('.edit-product-name').value=btn.dataset.name||'';r.querySelector('.edit-product-image').value=btn.dataset.image||'';r.querySelector('.edit-product-search').value=`${btn.dataset.code||''} · ${btn.dataset.name||''}`;r.querySelector('.edit-unit-price').value=Number(btn.dataset.price||0);r.querySelector('.edit-product-suggestions').classList.add('hidden');editOrderRecalc();
  };
  ...
  function itemRow(i=null){
    ...
    return `<div class="edit-order-item-row grid md:grid-cols-12 gap-2 p-3 bg-gray-50 rounded-xl" data-item-id="${esc(i?.id||'')}" data-linked="${linked?'1':'0'}">
      <div class="md:col-span-5 relative"><label class="text-[10px] font-semibold text-gray-500">Product Code / Item</label><input ${linked?'disabled':''} value="${esc(productText)}" class="edit-product-search mt-1 w-full border rounded-lg px-3 py-2 bg-white disabled:bg-gray-100" placeholder="Type product code or item name..." onfocus="showEditProductSuggestions(this)" oninput="editProductInputChanged(this)"><input type="hidden" class="edit-product-id" value="${esc(i?.product_id||'')}"><input type="hidden" class="edit-product-code" value="${esc(i?.product_code_snapshot||'')}"><input type="hidden" class="edit-product-name" value="${esc(i?.item_name_snapshot||'')}"><input type="hidden" class="edit-product-image" value="${esc(i?.image_url_snapshot||'')}"><div class="edit-product-suggestions hidden absolute z-[100] left-0 right-0 top-full mt-1 max-h-64 overflow-y-auto bg-white border rounded-xl shadow-xl"></div>${linked?'<div class="text-[9px] text-blue-600 mt-1">Linked to Procurement — product and qty are locked until unlinked.</div>':''}</div>
  ...
  window.openEditSalesOrder=async function(orderId){
    ...
    openModal(`Edit ${docNo(o)}`,`<form id="editSalesOrderForm" class="space-y-5">
      ...
      <div><div class="flex items-center justify-between mb-2"><div><h4 class="font-bold">Items</h4><div class="text-[10px] text-gray-400">You can correct qty, price and discount, or add another item.</div></div><button type="button" onclick="addEditOrderItem()" class="px-3 py-2 border rounded-lg text-xs font-semibold">+ Add Item</button></div><div id="editOrderItems" class="space-y-3">${editState.items.map(itemRow).join('')}</div></div>
  ...
  async function saveEditSalesOrder(e){
    ...
    for(const r of rows){
      const id=r.dataset.itemId||null,linked=r.dataset.linked==='1';const row={product_id:r.querySelector('.edit-product-id').value||null,product_code_snapshot:r.querySelector('.edit-product-code').value||null,item_name_snapshot:r.querySelector('.edit-product-name').value||null,image_url_snapshot:r.querySelector('.edit-product-image').value||null,qty:Number(r.querySelector('.edit-qty')?.value||1),unit_price:Number(r.querySelector('.edit-unit-price').value||0),discount_amount:Number(r.querySelector('.edit-line-discount').value||0),source_type:f==='pre_order'?'pre_order':'stock'};
      if(!row.product_id||row.qty<1||row.unit_price<0||row.discount_amount<0)return showToast('Check all item product, qty, price and discount values.','err');
      ...
      else{row.sales_order_id=o.id;row.fulfillment_status=f==='pre_order'?'pending':'ready';const x=await db.from('sales_order_items').insert(row);if(x.error)return showToast(x.error.message,'err')}
    }
