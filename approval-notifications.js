...
(function(){
  ...
  function dashboardCard(){
    ...
    box.innerHTML=
      ...
      +'<div class="grid grid-cols-3 gap-3 lg:min-w-[520px]">'
  ...
  function requestHtml(r,type){
    ...
    var title=order?(r.document_no||'Order Edit'):(r.customer_name||'Customer');
    var subtitle=order
    ...
    return '<button type="button" onclick="'+click+'" class="w-full text-left rounded-xl border bg-white p-4 hover:bg-amber-50/40">'
      ...
      +'<span class="px-2 py-1 rounded-md text-[9px] font-bold '+(order?'bg-blue-50 text-blue-700':'bg-green-50 text-green-700')+'">'+(order?'ORDER EDIT':'CUSTOMER')+'</span>'
      +'<b>'+esc(title)+'</b></div><div class="text-xs text-gray-500 mt-1">'+esc(subtitle)+'</div>'
      +'<div class="text-xs text-gray-700 mt-2">'+esc(r.request_note||'No request note')+'</div></div>'
  ...
  window.openApprovalCenter=async function(filter){
    ...
    var rows=[];
    if(filter!=='customers')A.orders.forEach(function(x){rows.push({type:'order',x:x})});
    if(filter!=='orders')A.customers.forEach(function(x){rows.push({type:'customer',x:x})});
    ...
    var html='<div class="space-y-4">'
      ...
      +'</div>'
      +'<div class="rounded-xl border border-blue-100 bg-blue-50 p-3 text-xs text-blue-800">Open a request to review it, make any final adjustment, then approve/publish or reject.</div>'
  ...
  window.renderApprovals=async function(){
    ...
    await loadPending();
    var rows=[];
    A.orders.forEach(function(x){rows.push({type:'order',x:x})});
    A.customers.forEach(function(x){rows.push({type:'customer',x:x})});
    rows.sort(function(a,b){return new Date(b.x.requested_at)-new Date(a.x.requested_at)});
  ...
  var oldOrderReview=window.reviewSalesEditRequest;
  if(typeof oldOrderReview==='function'){
    window.reviewSalesEditRequest=async function(){
      var out=await oldOrderReview.apply(this,arguments);
      setTimeout(refresh,50);
      return out;
    };
  }

  var oldCustomerReview=window.reviewCustomerRequest;
  if(typeof oldCustomerReview==='function'){
    window.reviewCustomerRequest=async function(){
      var out=await oldCustomerReview.apply(this,arguments);
      setTimeout(refresh,50);
      return out;
    };
  }
