...
(function(){
  ...
  window.openEditCustomer=async function(id){
    ...
    document.getElementById('multiEditCustomerForm').onsubmit=async e=>{
      e.preventDefault();
      const contacts=readContacts(),firstPhone=contacts.find(x=>x.contact_type==='phone')?.contact_value||null,firstEmail=contacts.find(x=>x.contact_type==='email')?.contact_value||null;
      const patch={name:document.getElementById('mecName').value.trim(),customer_code:document.getElementById('mecCode').value.trim()||null,customer_since:document.getElementById('mecCustomerSince').value||null,address:document.getElementById('mecAddress').value.trim()||null,notes:document.getElementById('mecNotes').value.trim()||null,phone:firstPhone,email:firstEmail,updated_at:new Date().toISOString()};
